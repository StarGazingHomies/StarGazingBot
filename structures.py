...
class VerifyResponse(object):
    def __init__(self, message):
        self.wait = Embed(title = "User Verification", description="React with <a:animated_check:717397008922443846> to verify!", color = Colour.blue(),)
        self.timeout = Embed(title = "Timed out!", color=Colour.red(), )
        self.success = Embed(title = "User Verification", description=f"<@!{message.author.id}> You have been verified! Access to the rest of the server will be granted shortly.", color=Colour.green(),)
        self.fail = Embed(title = "User Verification", description=f"<@!{message.author.id}> Please react with the right emoji.", color=Colour.red(), )
