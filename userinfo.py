...
class SettingsManager(object):
    ...
    def load_servers(self, *args, **kwargs):
        ...
        with os.scandir(self.serverpath) as settingsdir:
            for file in settingsdir:
                try:
                    server = yaml.safe_load(open(os.path.join(self.serverpath, file),'r'))
    ...
    def server_edit(self, server, setting, value, autosave = True):
        ...
        if not SettingTypeCheck(server, setting, value):
