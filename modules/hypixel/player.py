...
Identifier - Either the username or the uuid"""
    ...
    def skyblock(self, session):
        if self.data[1] is None:
            self.get(session)
        parse_skyblock(self, self.data[1])
    ...
    def parse_skyblock_singleprofile(self, data):
        ...
        for skill_type in SKYBLOCK_DICT["SKILLNAMES"]:
            try:
                ...
                logger.debug(f"The user {self.username} has not unlocked the skill {skill_type}, or their API isn't on.")
                self.partialAPIoff = True
                break
        ...
        if self.partialAPIoff:
            logger.debug(f"Since some data from user {self.username} are missing, getting data from achievements instead.")
