...
##        self.your_youre_check.enabled_rules = 'YOUR_YOU_RE'
    ...
    async def on_message(self, message):
        ...
        try:
            lastmsgtime, lastpoints = self.lastmessage[serverid][authorid]
            if time.time() - lastmsgtime > 75:
                self.add_points(serverid, authorid, r)
                self.lastmessage[serverid][authorid] = (time.time(), r)
            elif r > lastpoints:
                self.add_points(serverid, authorid, r-lastpoints)
                self.lastmessage[serverid][authorid] = (lastmsgtime, r)
        except KeyError:
            try:
                self.add_points(serverid, authorid, r)
                self.lastmessage[serverid][authorid] = (time.time(), r)
